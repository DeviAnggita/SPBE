...
<body>
    ...
    <section class="bg-half-170 bg-light d-table w-100">
        <div class="container">
            <div class="row mt-5 justify-content-center">
                <div class="col-lg-12 text-center">
                    <div class="pages-heading">
                        <h4 class="title mb-0"> Inovasi </h4>
                    </div>
                </div>
                <!--end col-->
            </div>
            ...
            <div class="row justify-content-center" style="position: relative; top: 2cm">
                <div class="col-md-10">
                    <div class="text-center subcribe-form">
                        <form style="max-width: 800px;" action="{{ route('search.inovasi') }}" method="GET">
                            <div class="mb-0">
                                <input type="text" id="help" name="search" value="{{ $search }}"
            ...
            <div class="position-breadcrumb">
                <nav aria-label="breadcrumb" class="d-inline-block">
                    <ul class="breadcrumb bg-white rounded shadow mb-0 px-4 py-2">
                        ...
                        <li class="breadcrumb-item text-primary">Search</li>
                    </ul>
                </nav>
            </div>
        </div>
    ...
    <section class="section">
        <div class="container">
            <div class="row">
                <div class="text-end">
                    <a href="{{ route('inovasiuser.create') }}" class="btn btn-primary">Add Inovasi</a>
                </div>
            </div>
                ...
                <div class="col-12 filters-group-wrap">
                    <div class="filters-group">
                        <ul class="container-filter list-inline mb-0 filter-options text-center">
                            <li onclick="handleAddHidden()"
                                class="list-inline-item categories-name border text-dark rounded active"
                                data-group="all">All</li>
                            <li onclick="handleRemoveHidden()"
                                class="list-inline-item categories-name border text-dark rounded"
                                data-group="{{ Auth::user()->id_user }}">My Inovasi</li>
                            <li onclick="handleAddHidden()"
                                class="list-inline-item categories-name border text-dark rounded"
                                data-group="Infrastruktur">Infrastruktur</li>
                            <li onclick="handleAddHidden()"
                                class="list-inline-item categories-name border text-dark rounded"
                                data-group="Aplikasi">Aplikasi</li>
                            <li onclick="handleAddHidden()"
                                class="list-inline-item categories-name border text-dark rounded"
                                data-group="Keamanan">Keamanan</li>
                        </ul>
                    </div>
                </div>
                <!--end col-->
            ...
            <div id="grid" class="row">
                @foreach ($inovasis as $inovasi)
                    <div class="col-lg-4 col-md-6 col-12 mt-4 pt-2 picture-item"
                        data-groups='["{{ $inovasi->kategori_inovasi }}", "{{ $inovasi->id_user }}"]'>
                        <div
                            class="card blog border-0 work-container work-primary work-classic shadow rounded-md overflow-hidden">
                            <div class="position-relative">
                                <img src="../assets/cover/{{ $inovasi->cover }}" class="card-img-top rounded-top"
                                    alt="...">
                                <div class="overlay rounded-top"></div>
                            </div>
                            <div class="card-body" style="height: 5.5cm">
                                <div class="content">
                                    @if ($inovasi->kategori_inovasi == 'Infrastruktur')
                                        <a href="javascript:void(0)"
                                            class="badge badge-link bg-primary">{{ $inovasi->kategori_inovasi }}</a>
                                    @elseif ($inovasi->kategori_inovasi == 'Aplikasi')
                                        <a href="javascript:void(0)"
                                            class="badge badge-link bg-success">{{ $inovasi->kategori_inovasi }}</a>
                                    @else
                                        <a href="javascript:void(0)"    
                                            class="badge badge-link bg-warning">{{ $inovasi->kategori_inovasi }}</a>
                                    @endif
                                    <h5 class="mt-3"><a
                                            href="{{ route('inovasiuser.show', $inovasi->id_inovasi) }}"
                                            class="text-dark title">{{ $inovasi->judul_inovasi }}</a></h5>
                                </div>
                            </div>
                            <div class="text-end mb-2">
                                <a href="{{ route('inovasiuser.show', $inovasi->id_inovasi) }}"
                                    class="link h6">Read More <i class="uil uil-angle-right-b align-middle"></i></a>
                            </div>
                            <div class="author">
                                <small class="text-light user d-block"><i class="uil uil-user"></i>
                                    {{ $inovasi->nama_pegawai }}</small>
                                <small class="text-light date"><i class="uil uil-calendar-alt"></i>
                                    {{ $inovasi->publish_at }}</small>
                            </div>
                        </div>
                    </div>
                    <!--end col-->
                @endforeach
                @foreach ($inovasismember as $inovasi)
                    <div class="myinovasi-show col-lg-4 col-md-6 col-12 mt-4 pt-2 picture-item"
                        data-groups='["{{ $inovasi->kategori_inovasi }}", "{{ $inovasi->id_user }}"]'
                        id="inovasiMember" hidden>
                        <div
                            class="card blog border-0 work-container work-primary work-classic shadow rounded-md overflow-hidden">
                            <div class="position-relative">
                                <img src="../assets/cover/{{ $inovasi->cover }}" class="card-img-top rounded-top"
                            ...
                            <div class="author">
                                <div class="d-flex">
                                    <small class="text-light user d-block me-2"><i class="uil uil-user"></i>
                                        {{ $inovasi->nama_pegawai }}</small>
                                    <small class="badge badge-link bg-info">{{ $inovasi->keterangan }}</small>
                                </div>
                                <small class="text-light date"><i class="uil uil-calendar-alt"></i>
                                    {{ $inovasi->publish_at }}</small>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-12 mb-0 mb-md-4 pb-0 pb-md-2">
                                <a href="{{ route('home.index') }}" class="logo-footer">
                                    <img src="../assets/user/images/kominfo-light.png" height="60" width="200" alt="">
                                </a>
                                <p class="mt-4">Dinas Komunikasi dan Informatika Kabupaten Sukoharjo adalah Organisasi Perangkat Daerah yang di bentuk untuk menyelenggarakan urusan Pemerintahan bidang komunikasi dan informatika, bidang persandian, dan bidang statistik.</p>
                                <ul class="list-unstyled social-icon foot-social-icon mb-0 mt-4">
                                    <li class="list-inline-item"><a href="javascript:void(0)" class="rounded"><i
                                                data-feather="facebook" class="fea icon-sm fea-social"></i></a></li>
                                    <li class="list-inline-item"><a href="https://www.instagram.com/diskominfo.skh/" class="rounded"><i
